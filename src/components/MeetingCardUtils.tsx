...
import { getSharedLanguages, class2Hex } from "@/utils/meetingUtils";
...
export const useMeetingCardUtils = (meetingWithPeer: MeetingWithPeer, textColor: string, t: TFunction, locale: string = 'en') => {
  ...
  const formatDateForDisplay = (date: Date) => {
    if (isToday(date)) {
      return `${t('today')} ${format(date, 'HH:mm', { locale: dateFnsLocale })}`
    }
  ...
  const getFirstSlotDay = () => {
    ...
    return format(firstSlotDate, 'MMM d', { locale: dateFnsLocale });
  }
  ...
  const groupTimeSlotsByDay = () => {
    ...
    sortedSlots.forEach(timestamp => {
      ...
      const dayKey = isToday(date) ? t('today') : format(date, 'EEE dd MMM', { locale: dateFnsLocale })
  ...
  function MeetingLanguagesChips({ meetingColor, chipSx }: { meetingColor: string, chipSx: SxProps }) {
    ...
    return (
      <div className="flex items-center justify-center gap-2">
        <LanguageIcon sx={{ color: class2Hex(meetingColor) }} fontSize="small" />
